...
{
public:
	Tutorial_AlchemiUI(int maxCount);
	~Tutorial_AlchemiUI();
...
/// マシンの選択時メニュー内の操作を行う
/// </summary>
class Tutorial_MachineExplanation : public ICommand_Tutorial
{
public:
	/// <summary>
	/// マシンの選択時メニュー内の操作
...
/// ゲージへの視線誘導を行う
/// </summary>
class Tutorial_ResourceGauge : public ICommand_Tutorial
{
public:
	/// <summary>
	/// マシンの選択時メニュー内の操作
	/// </summary>
	/// <param name="type">0 = HP ,1 = MP , 2 = Crystal</param>
	Tutorial_ResourceGauge(int type);
	~Tutorial_ResourceGauge();
...
private:
	...
	int m_resourceType;					// リソースゲージの視線誘導先を決定

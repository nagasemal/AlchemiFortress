...
ICommand_Tutorial* TutorialManager::GenerateCommand(Tutorial_Status type)
{
	...
	case TutorialCommander::ALCHEMI_UI:
		...
		return new Tutorial_AlchemiUI(type.val);

	// リソースゲージ用コマンド
	case TutorialCommander::HP_UI:
	case TutorialCommander::MP_UI:
	case TutorialCommander::CRYSTAL_UI:

		return new Tutorial_ResourceGauge(type.type - (int)TutorialCommander::HP_UI);

	//マシンメニュー用コマンド
	case TutorialCommander::REPEA_UI:

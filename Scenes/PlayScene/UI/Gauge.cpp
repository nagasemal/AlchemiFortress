...
void Gauge::Initialize()
{
    ...
    SimpleMath::Vector2 numberPos = 
        ...
        m_base_Hp->GetTexture_W() / numRender_rate * uiData.rage.x,
        m_base_Hp->GetTexture_H() / numRender_rate * uiData.rage.y);

    m_resourceRenderHP = std::make_unique<Number>(numberPos, SimpleMath::Vector2(uiData.option["NUMBER_RAGE"]));
    m_resourceRenderHP->SetColor(SimpleMath::Color(Colors::White));
...
void Gauge::TransparentUI(float transparentVal)
{
    ...
    m_gauge_Hp      ->SetColor(m_gauge_Hp       ->GetColorRGB(), transparentVal);
    ...
    m_resourceRenderHP      ->SetColor(SimpleMath::Color(m_resourceRenderHP     ->GetColorRGB(), transparentVal));
